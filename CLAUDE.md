...
- **Any new persisted field on `store` needs an explicit default in
  ...
  onboarding).
- **Do not touch the leveling system, mastery ladder, or points mechanics.**
  Frozen — see **Frozen work** for the exact scope.
...
**Leveling system redesign — on hold.** Madison's own words:

> "Do not change anything relevant to the level up system yet yet, do not
> change anything like that. Do everything else but let's not implement that
> yet. Keep the mastery ladder and all that stuff in the file you send me
> next. That is going to a large batch I've changed directions and figured out
> how we are gonna do this now. We will come back to this."

`TIER_UNLOCKS`, point-awarding and tier-unlock logic are frozen until she
reopens it. One narrow exception already granted: the points-fly animation's
*timing* is fine to adjust, being unrelated to the maths. Track that boundary
exactly — don't round it off in either direction, and resist refactoring
...
- Say what was found or changed beyond the literal ask — a fix that touched a
  ...
  effect.
- Respect scope boundaries precisely, including narrow exceptions carved out of
  a broader freeze.
